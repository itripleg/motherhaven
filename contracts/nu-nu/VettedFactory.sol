...
contract TokenFactory is Ownable, ReentrancyGuard {
    ...
    event TokenFundingGoalUpdated(address indexed token, uint256 oldGoal, uint256 newGoal);
    ...
    modifier validToken(address tokenAddress) {
        require(tokens[tokenAddress] != TokenState.NOT_CREATED, "Invalid token");
        _;
    }
     ...
     * @param newGoal The new funding goal in wei.
     */
    function setTokenFundingGoal(address tokenAddress, uint256 newGoal) external onlyOwner validToken(tokenAddress) {
        require(newGoal > 0, "Invalid funding goal");
        require(tokens[tokenAddress] == TokenState.TRADING || tokens[tokenAddress] == TokenState.RESUMED, "Token not in valid state");
        require(collateral[tokenAddress] < fundingGoals[tokenAddress], "Goal already reached");
     ...
     */
    function _calculatePrice(uint256 effectiveSupplyStart, uint256 effectiveSupplyEnd) internal view returns (uint256) {
        ...
        uint256 costNumerator = INITIAL_PRICE * tokenAmount * (2 * supplyRange + scaledSum);
     ...
     */
    function calculateBuyPrice(address tokenAddress, uint256 tokenAmount) public view returns (uint256) {
        uint256 effectiveSupplyStart = virtualSupply[tokenAddress] - INITIAL_MINT;
     ...
     */
    function calculateSellPrice(address tokenAddress, uint256 tokenAmount) public view returns (uint256) {
        uint256 effectiveSupplyStart = virtualSupply[tokenAddress] - INITIAL_MINT;
        require(effectiveSupplyStart >= tokenAmount, "Cannot sell more than effective supply");
        uint256 effectiveSupplyEnd = effectiveSupplyStart - tokenAmount;
        return _calculatePrice(effectiveSupplyStart, effectiveSupplyEnd);
    }
     ...
     */
    function calculateTokensForETH(address tokenAddress, uint256 ethAmount) internal view returns (uint256) {
        uint256 low = 0;
     ...
     * @return The amount of tokens that can be purchased (in base units).
     */
    function calculateTokenAmount(address tokenAddress, uint256 ethAmount) public view returns (uint256) {
     ...
     */
    function createToken(string calldata name, string calldata symbol, string calldata imageUrl, address burnManager) external returns (address) {
        ...
        Token token = new Token(address(this), msg.sender, name, symbol, imageUrl, burnManager);
        ...
        emit TokenCreated(tokenAddress, name, symbol, imageUrl, msg.sender, defaultFundingGoal, burnManager);
        return tokenAddress;
    }
     ...
     */
    function buy(address tokenAddress) external payable nonReentrant validToken(tokenAddress) {
        require(tokens[tokenAddress] == TokenState.TRADING || tokens[tokenAddress] == TokenState.RESUMED, "Not trading");
        require(msg.value >= MIN_PURCHASE && msg.value <= MAX_PURCHASE, "Invalid amount");
        
        uint256 fee = calculateFee(msg.value);
        uint256 purchaseAmount = msg.value - fee;

        uint256 tokensToMint = calculateTokenAmount(tokenAddress, purchaseAmount);
        require(tokensToMint > 0, "ETH amount too low to buy any tokens");

        Token token = Token(tokenAddress);
        require(virtualSupply[tokenAddress] + tokensToMint <= MAX_SUPPLY, "Max supply");
        
        uint256 newBalance = token.balanceOf(msg.sender) + tokensToMint;
        require(newBalance <= (MAX_SUPPLY * MAX_WALLET_PERCENTAGE) / 100, "Exceeds max wallet");
        ...
        emit TokensPurchased(tokenAddress, msg.sender, tokensToMint, msg.value, fee);
     ...
     * @param tokenAmount The amount of tokens to sell (in base units).
     */
    function sell(address tokenAddress, uint256 tokenAmount) external nonReentrant validToken(tokenAddress) {
        require(tokens[tokenAddress] == TokenState.TRADING || tokens[tokenAddress] == TokenState.RESUMED, "Not trading");
        ...
        require(collateral[tokenAddress] >= grossAmount, "Insufficient token collateral");
        ...
        emit TokensSold(tokenAddress, msg.sender, tokenAmount, grossAmount, fee);
    }
    ...
    function resumeTrading(address tokenAddress) external {
        ...
        require(tokens[tokenAddress] == TokenState.GOAL_REACHED, "Not ready to resume");
