...
import { ExternalLink, Coins, TrendingUp, TrendingDown, Plus } from "lucide-react";
...
export function EventWatcher() {
  ...
  const showEventToast = useCallback((event: TokenEvent) => {
    const now = Date.now();
    ...
    switch (eventName) {
      case "TokenCreated":
        ...
        description = `${formatAddress(data.creator)} created ${data.name} (${data.symbol})`;
        icon = <Plus className="h-4 w-4 text-green-500" />;
        break;

      case "TokensPurchased":
        const ethAmountPurchased = data.price ? formatEther(data.price) : "0";
        const tokenAmountPurchased = data.amount ? formatEther(data.amount) : "0";
        title = "📈 Token Purchase";
        description = `${formatAddress(data.buyer)} bought ${formatNumber(tokenAmountPurchased)} tokens for ${formatNumber(ethAmountPurchased)} AVAX`;
        icon = <TrendingUp className="h-4 w-4 text-green-500" />;
        break;

      case "TokensSold":
        const ethSold = data.ethAmount ? formatEther(data.ethAmount) : "0";
        const tokensSold = data.tokenAmount ? formatEther(data.tokenAmount) : "0";
        title = "📉 Token Sale";
        description = `${formatAddress(data.seller)} sold ${formatNumber(tokensSold)} tokens for ${formatNumber(ethSold)} AVAX`;
        icon = <TrendingDown className="h-4 w-4 text-red-500" />;
        break;

      case "TradingHalted":
        title = "⏸️ Trading Halted";
        description = `Trading halted for token ${formatAddress(data.token || data.tokenAddress)}`;
