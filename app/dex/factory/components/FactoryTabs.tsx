...
export function FactoryTabs({
  ...
  const tabs = [
    ...
    },
    {
      ...
      status: getTabStatus("tokenomics"),
    },
    {
      id: "preview",
      label: "Preview",
  ...
  return (
    <Tabs value={activeTab} onValueChange={onTabChange} className="space-y-6">
      {/* Tab Navigation */}
      <TabsList className="unified-card p-2 border-primary/20 w-full h-auto">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <TabsTrigger
                ...
                disabled={isDisabled}
                className={`
                  relative flex flex-col items-center gap-2 p-4 rounded-lg transition-all duration-300 min-h-[80px]
                  data-[state=active]:bg-primary data-[state=active]:text-primary-foreground
                  disabled:opacity-50 disabled:cursor-not-allowed
                  ${!isActive && !isDisabled ? "hover:bg-primary/10" : ""}
                `}
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId="activeTab"
                    className="absolute inset-0 border-2 border-primary-foreground/20 rounded-lg"
